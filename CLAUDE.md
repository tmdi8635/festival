...
**공고 하나 = 행사 하나**이고, 지원은 그 안의 **포지션 하나**에 한다. 행사당 살아 있는
지원은 하나뿐이다(바꾸려면 취소 후 재지원). 성별 · 보건증 조건은 포지션이 갖고,
지원 가능 여부는 서버의 `resolvePositionBlock`이 목록 버튼과 지원 요청에 똑같이 판정한다.
회원은 **본인 성별로 공고가 걸러지고**(남: 무관+남성만 / 여: 무관+여성만), 비회원은 직접 고른다.
지원을 확정하면 그 포지션의 **발주가 있는 모든 근무일**에 배치된다.
