...
**공고 하나가 행사 하나다.** 그 안의 모집 줄(A타임 전일 · A타임 09.13 급구 · 인형탈 …)마다
시각 · 금액 · 성별 · 보건증 조건 · 날짜가 달라서, 상세에서 **줄을 골라 지원**한다. 여러 줄에
걸어 둘 수 있고 확정은 날짜마다 하나다. 회원은 본인 성별로, 비회원은 고른 성별로 공고가 걸러진다.

연일 행사의 줄은 **전일 참여** 또는 **날짜 골라 지원**이다. 전일 줄은 하루라도 겹치는 사람에게
잠기고, 날짜 줄은 겹친 날만 잠긴 칩에서 나올 날을 고른다(하단 시트). 급구 줄은 붉은 테두리 ·
배지로 먼저 눈에 걸리게 한다 — 하루만 되는 사람이 찾는 자리가 바로 거기다.
...
| 검토 대기 지원 2건 (서로 다른 행사, 하나는 날짜 줄에 일부 날만 낸 지원) | `db/recruit.ts` |
| 야시장 공고의 급구 줄 (A타임 마지막 날 1명) | `db/recruit.ts` |
| 응답 대기 근무 제안 2건 (전일 1 · 분할 1) + 같은 행사 다른 사람의 대기 · 거절 건 | `db/offer.ts` |
