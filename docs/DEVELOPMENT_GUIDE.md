...
## 11. 직무는 시스템이, 단가는 설정이 정한다 ★

**직무 목록은 코드에 고정돼 있다.** (`JOB_ROLE_CATALOG`, `src/type/staff.ts`)

직무는 우리끼리 쓰는 말이 아니라 **대행사와 주고받는 말**이다. 대행사가 직무별
인원수로 견적을 요청하면 우리가 직무별 단가로 답한다. 에이전시가 직무를 자유롭게
만들 수 있으면 '부팀장' · '서브팀장' 같은 내부 호칭이 견적서에 올라가고, 받는 쪽은
그게 무슨 자리인지 알 수 없다. 그래서 이름 · 순서는 시스템이 갖고,
**기준 설정이 갖는 것은 금액과 사용 여부뿐이다.**

```tsx
// ❌ 라벨을 직접 만들지 않는다
const label = role === "SUPERVISOR" ? "팀장" : "스태프";

// ✅ 카탈로그 + 기준 설정을 합쳐 둔 스토어에서 꺼낸다
import {
  useJobRoleLabel,
  useJobRoleOptions,
...
- `JobRole`은 `JOB_ROLE_CODES`의 **유니온 타입**이다. 아무 문자열이나 들어갈 수 없다.
  폼 · 쿼리스트링처럼 밖에서 온 값은 `isJobRole()`로 걸러 쓰고,
  zod 스키마는 `z.enum(JOB_ROLE_CODES)`로 검증한다.
- 새 직무는 **카탈로그에 추가해서** 늘린다. 화면에는 추가 · 삭제 · 이름 수정이 없다.
  기준 설정에 없는 코드는 `mergeJobRoles()`가 기본 단가로 채우므로,
  카탈로그에 한 줄 넣으면 모든 화면에 바로 나타난다.
- 저장은 `sanitizeJobRoles()`를 거친다. 카탈로그에서 온 이름 · 순서까지 같이 저장하면
  사본이 남아, 나중에 시스템이 이름을 고쳐도 옛 이름이 따라다닌다.
  목업 핸들러(`PUT /admin/settings`)도 같은 함수로 요청을 정리한다 — **막는 책임은 서버다.**
- 단가는 두 가지다. **지급 단가**(`defaultWage`, 인력에게 주는 돈)와
  **청구 단가**(`billingRate`, 대행사에 부르는 견적)다. 둘 다 행사 등록 시
  초기값으로 깔리고 행사마다 고칠 수 있다. 청구 단가는 **거래처가 갖지 않는다** —
  단가를 부르는 쪽이 에이전시라서다.
- **직무를 나열하는 모든 곳은 카탈로그의 순서를 따른다.**
  ...
  **내부 코드의 알파벳순**이고, 그래서 1번인 팀장(`SUPERVISOR`)이
...
기타수당의 기본값은 **0이다.** 시스템이 알아서 붙이지 않는다.
예전에는 8시간 넘게 선 날마다 식대 10,000원을 자동으로 얹었는데, 식대를 주는지 ·
얼마인지는 발주마다 다르고, 무엇보다 근태를 고칠 때마다 재계산이 돌면서
담당자가 적어 넣은 금액을 계속 되돌려 놓았다.
...
| `WORK_DAY_ADDED` | 현장 요청 추가 투입 · 결원 대체 | **늘어난다** |
...
### 근무일 목록은 계약서가 아니라 **행사**가 정한다

재작성 화면의 '계약에 담을 근무일'에는 **행사 근무일(`event.days`) 전체**를 세우고,
지금 계약서에 적힌 날만 체크해 둔다. (`ContractAmendModal`)

계약서에 적힌 날만 세우면 늘어난 날을 체크할 자리가 아예 없다. 실제로 그래서
담당자는 계약서를 지우고 처음부터 다시 쓰고 있었고, 그러면 **왜 늘었는지가 이력에서
사라진다.** 행사가 3일인데 이틀만 계약한 사람에게 하루를 더 부탁하는 일은
중도 하차만큼 흔하다.

배치도 함께 움직인다. **세 가지가 같이 가지 않으면 근거 없는 지급이 된다.**

| 조작 | 배치 | 정산 |
|---|---|---|
| 체크를 푼다 | 취소 (`cancelsRemovedAssignments`) | 지급 대상에서 빠진다 |
| 새로 켠다 | 되살리거나 새로 만든다 | 지급 대상에 들어간다 |

- 새로 만드는 배치의 금액은 `resolveAssignmentWage()`가 정한다.
  인력 배치 화면과 **같은 함수**를 써야 같은 날 같은 직무에 금액이 다른 배치가 안 생긴다.
- 행사 근무일 밖의 날짜는 서버가 거부한다. 행사에 없는 날의 계약은 근거가 없고
  정산 건도 만들어지지 않는다.
- 늘어난 날은 `addedWorkDates`로 남아 계약서 본문에 찍힌다.
  무엇이 빠졌는지만 남기면 총액이 왜 **올랐는지**를 문서가 설명하지 못한다.
