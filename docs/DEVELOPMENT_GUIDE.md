...
| 상태 | 뜻 |
|---|---|
| 미발급 | 확정 배치는 됐는데 계약서가 없다. **가장 먼저 보여야 한다** |
| 작성됨 · 발송됨 | 진행 중 |
| 반려 · 기한만료 | 손이 더 가야 한다. 완료된 것보다 뒤에 묻히면 안 된다 |
| 서명완료 | 끝 |
| 재작성됨 | 지나간 문서. 사람의 현재 상태는 **가장 높은 차수**로 본다 |

- 계약서는 **사람 한 명당 한 장**이다. 여러 날 나오는 사람도 한 장에 근무일이 모두 담긴다.
  날짜마다 만들면 서명도 날짜 수만큼 받아야 한다.
- **발급은 전체와 직무별을 모두 지원한다.** 한 행사 안에서도 직무마다 계약 조건이
  다르다. (팀장은 책임 범위, 설치는 일급과 안전 조항) 전체 발급만 있으면
  첫 직무의 템플릿으로 전원이 묶여 나가고 나머지는 손으로 다시 만들어야 한다.
- 대상이 없으면 **이유를 돌려준다.** 조용히 0건을 주면 눌러도 아무 일이 없는 화면이 된다.
...
| **서버** (`requirePermission`) | **막는다** | 앞의 둘은 실수를 줄이는 장치일 뿐이다 |
