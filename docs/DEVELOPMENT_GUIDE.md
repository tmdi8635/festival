...
이 도구는 **서버가 생기기 전까지 사내에서 먼저 쓴다.** 그래서 자료를 통째로 들고
나갈 수 있는 길은 기능 하나가 아니라 전제 조건이다. 옮길 수 없는 자료는 쌓일수록 발이 묶인다.

- 파일 하나에 전부 담는다. (`src/type/backup.ts`의 `BackupFile`)
  도메인별로 쪼개면 받는 쪽이 순서를 맞춰야 하고 — 거래처 → 인력 → 행사 → 계약 · 정산 —
  한 파일이라도 빠지면 참조가 끊긴다.
- **도메인 배열을 새로 만들면 `BackupData`와 `mocks/db/backup.ts`의 `COLLECTIONS`에 반드시 넣는다.**
  빠뜨리면 조용히 백업에서 사라진다. 두 곳의 키가 어긋나면 타입이 먼저 막아 준다.

### 버전은 두 개다

| 값 | 언제 올리나 | 무엇을 판단하나 |
|---|---|---|
| `BACKUP_SCHEMA_VERSION` | **필드를 지우거나 뜻을 바꿀 때만** | 지금 코드가 이 파일을 읽을 수 있는가 |
| `APP_VERSION` | 배포마다 | 어느 배포에서 뽑았는지 추적 |

필드를 **더하는** 것으로는 구조 버전을 올리지 않는다. 옛 파일에 새 필드가 없는 건
기본값으로 메우면 된다. 앱 버전으로 판단하면 아무것도 안 바뀐 배포마다 멀쩡한 백업이 거부된다.

- 판정은 `inspectBackupFile()` 하나에서 한다. 화면(미리 알려 주기)과 핸들러(실제로 막기)가
  같은 함수를 쓴다. 둘이 갈라지면 화면은 되는데 저장이 안 되는 상태가 생긴다.
- **버전을 자료 모양보다 먼저 본다.** 더 새로운 앱이 만든 파일은 구조가 다른 게 당연한데,
  모양부터 따지면 "거래처 · 인력…이 없습니다"라는 엉뚱한 이유를 대게 된다.
- **meta의 건수를 믿지 않고 다시 센다.** 손으로 고친 파일이 흔하다.

### 가져오기는 되돌릴 수 없다

- 배열을 새로 대입하지 않고 **제자리에서 비우고 채운다**(`splice`). 핸들러 · 화면이 이미
  그 배열을 참조하고 있어서, 새 배열로 바꾸면 참조를 든 쪽은 옛 배열을 계속 본다.
  (가져오기가 된 것처럼 보이는데 목록은 그대로인 상태가 만들어진다)
- 기준 설정은 객체 하나다. 병합하지 말고 **키를 다 지운 뒤** 넣는다. 병합하면 없앤 직무가 되살아난다.
- 성공 뒤 `queryClient.invalidateQueries()`로 **캐시를 통째로 버린다.** 쿼리키를 하나씩
  무효화하면 빠뜨린 화면이 옛 자료를 계속 보여 주는데, 그게 정산 금액이나 배치 명단이면
  없는 사람에게 돈을 보내는 일이 된다.
- 가져오기 기록은 **갈아 끼운 뒤에** 남긴다. 운영 로그도 백업 대상이라 먼저 남기면
  방금 들어온 자료가 그 줄을 덮어써서 "통째로 바꾼 일"만 흔적 없이 사라진다.
