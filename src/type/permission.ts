...
export const PERMISSION_RESOURCES: Record<PermissionResource, ResourceDef> = {
  ...
  contract: {
    ...
    /*
      ...
      한동안 뺐던 권한이다. 계약서가 시스템 밖으로 나가지 않고 담당자가 종이로
      배부하던 동안에는 켜 놓아도 아무 일이 일어나지 않는 빈 스위치였기 때문이다.
      스태프 포털이 생기면서 실제로 문서가 본인에게 나가게 됐고, 그 순간부터
      "누가 근로자에게 계약서를 보낼 수 있는가"는 등록과 다른 질문이 된다.
      (보내면 계약번호가 붙고, 받는 사람에게는 그것이 정식 문서로 보인다)
    */
    description: "계약서 등록 · 발송 · 재작성 · 템플릿",
